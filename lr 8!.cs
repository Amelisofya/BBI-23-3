...
class Task_9 : Task
{
    ...
    private List<LetterPair> CreateRusLetterPairs()
    {
    ...
    private List<LetterPair> CreateEngLetterPairs()
    {
        List<LetterPair> pairs = new List<LetterPair>();
    ...
    private bool CheckTextOnLanguage()
    {
    ...
    protected override string ParseText(string text)
    {
        ...
        {
            letterPairs = CreateRusLetterPairs();
        }
        else
        {
            letterPairs = CreateEngLetterPairs();
        }

        for (int i = 0; i < text.Length - 1; i++)
        {
            string pair = text[i].ToString() + text[i + 1].ToString();
            var letterPair = letterPairs.FirstOrDefault(lp => lp.Pair == pair);
            if (letterPair.Pair != null)
            {
                letterPair.Increment();
            }
        }

        var sortedLetterPairs = letterPairs.OrderByDescending(lp => lp.Count).Take(10).ToArray();
        string[] ke = sortedLetterPairs.Select(lp => lp.Pair).ToArray();
...
class Program
{
    ...
    {
        string text = "Крокодил, бегемот! Обезяны, кашалоты. И веселый попугай. Мы любим ролы";
